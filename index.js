...
const init = async (client, tresshold) => {
  try {
    console.log("\n============================");
    ...
    await delay(3000);
    if (e.message !== "Tresshold Reached") {
      init(client);
    }
  }
};
...
const sellfun = async (client) => {
  ...
  console.log(
    getNowFormatDate(),
    `My Account Infos: ${userbalance2.SOL.available} $SOL | ${userbalance2.USDC.available} $USDC`
  );
  ...
  console.log(
    getNowFormatDate(),
    `Trade... ${quantitys} $SOL to ${(lastPriceask * quantitys).toFixed(
      2
    )} $USDC`
  );
  ...
  if (orderResultAsk?.status == "Filled" && orderResultAsk?.side == "Ask") {
    ...
    console.log(
      getNowFormatDate(),
      "Sold successfully:",
      `Order number:${orderResultAsk.id}`
    );
    init(client);
  } else {
...
const buyfun = async (client) => {
  ...
  let userbalance = await client.Balance();
  if (threshold >= userbalance.USDC.available) {
    throw new Error("Tresshold Reached");
  }
  let balanceSol = 0;
  if (userbalance.SOL) {
  ...
  console.log(
    getNowFormatDate(),
    `My Account Infos: ${balanceSol} $SOL | ${userbalance.USDC.available} $USDC`
  );
  ...
  if (orderResultBid?.status == "Filled" && orderResultBid?.side == "Bid") {
    ...
    console.log(
      getNowFormatDate(),
      "Bought successfully:",
      `Order number: ${orderResultBid.id}`
    );
    init(client);
  } else {
...
(async () => {
  ...
  console.log(
    "Before begin, make sure you have at least 10 USDC on your backpack account"
  );
  const tresshold = readlineSync.question(
    "set your USDC tresshold (When your USDC balance reach this, bot will stop) ? : "
  );

  const client = new backpack_client_1.BackpackClient(apisecret, apikey);
  init(client, tresshold);
})();
